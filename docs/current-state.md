...
- 桌面命令面：2026-08-02 在变更工作树运行 `node scripts/check-desktop-command-surface.mjs --mode all --json` 通过，Rust 注册、生产 TypeScript 源码与 Vite bundle 三个集合均为 45，动态调用、无法静态证明的调用与所有差异集均为 0。根 `check:desktop-command-surface` 和 `check:project` 保护 source 面，桌面 `build` 在 Vite 之后保护 bundle 面；精确入口、编译器符号约束与保留的迁移/恢复内核见 [实现证据索引](internal/implementation-inventory.md#4-桌面生产命令面与保留边界)。
...
- CodeGraph：Task 11 的历史快照是在实际变更工作树运行 `pnpm codegraph:index`、`pnpm codegraph:sync` 与 `pnpm codegraph:status` 后得到 216 个文件、3,006 个节点和 8,936 条边；全分支解析器加固完成后的 final fresh `sync/status` 同样通过，当前索引为最新状态，共 218 个文件、3,017 个节点和 8,931 条边。`.codegraph/` 是忽略的本机索引，没有作为产品或验收资产提交。
...
- 全分支最终复核与规模：Task 12 的初始 23/23 历史之后，终审用 TypeScript 编译器符号解析加固命令提取器，只接受来自 `@tauri-apps/api/core` 的具名 `invoke` 运行时绑定作为非可选直接调用；命名空间导入、值再导出、保存、转发、条件调用或其他非直接值用途一律产生带文件、行号和原因的可操作失败，type-only 导入、导出和纯类型引用不进入命令面。focused parser 51/51 与完整 `scripts/*.test.mjs` 53/53 fresh 通过，source/all 均为 `45/45/45` 且 `dynamicInvocations`、`unsupportedInvocations` 和四类差异为 0；完整 `pnpm check` 的 Rust 245 项、Vitest 195 项、release-manifest 2 项与 desktop/site build 通过。全部历史 Important 修复经 final fresh review 复核为 clean。相对实施计划 base `24e0fc79f9bda0401362b4e93cb570774b4abfa8`，最终 whole-branch 跟踪差异为 47 个文件、3,304 行新增、5,606 行删除，净减少 2,302 行；该统计包含 change 证据、门禁、测试和最终文档资产，不用作功能完成度或用户验收依据。
- 终审延后 Minor 裁决：`localeCompare` 只对小写 ASCII 命令诊断集合排序，输出不参与产品语义，在当前约束下可接受；本 branch 不新增 `open_project_preview` 直接写安全测试，因为该函数未改且打开路径只读取用户项目，已有 adoption/reset、Manifest 和受管源码快照测试继续保护写入边界。两项均非未修复产品缺陷。
...
- 无副作用烟测：Computer Use 以精确 `.app` 路径启动到“我的部署”为 `PASS`；5 条已有记录可见。首轮文件夹入口后 Mac 锁定，且按显示名读取曾命中另一同名 legacy 窗口及其既有数据库 toast；该窗口明确不是当前精确构建 app 的回归证据。解锁后 `list_apps` 显示两个 bundle id 同为 `cloud.finagent.abcdeploy` 的实例，后续状态读取和操作始终绑定上述 worktree 精确 `.app` 路径。精确实例初始 AX 为系统 `Open` sheet，按 Escape 回到新建页，证明文件夹选择器已经打开并安全取消，因此该项为 `PASS`，未选择目录。
- 主线入口补测：`wx` 的在线服务器详情恢复后，`更新上线` 进入服务器更新编辑器并完成预检，显示“可以开始 / 服务器连接已经验证”，未出现 `command not found`；未点击最终“更新上线”。`swifteng` 的既有待处理记录恢复并预填本地项目；选择“这台电脑”后显示“可以开始 / 这台电脑可以接收本次运行”，切换“服务器”后能看到已验证服务器与“连接新服务器”，选择已验证服务器后显示“可以开始 / 服务器连接已经验证”。未点击“在本机运行”或“上线到服务器”，没有启动服务、新建连接、上线或修改服务器。连接新服务器 modal 与准备说明正常展开；“查看购买服务器说明”的打开动作已派发，但按 Chrome 精确 bundle id 读取返回 `noWindowsAvailable`，目标窗口不可观察，仍为 `NOT_TESTED`。复制动作仅能在需要连接 Provider 的 `CnbSecretHandoffDialog` 授权流程中到达；为避免外部连接或副作用未进入，仍为 `NOT_TESTED`。
- 证据边界：Task 12 只新增 fresh 自动门禁、签名本地测试包和有限 smoke 证据，不新增用户验收。服务器正向主线仍保持原 `VERIFIED`；本机运行、更新部署和版本恢复仍为 `IMPLEMENTED_UNVERIFIED`。文件夹选择器取消、本机入口和服务器入口按解锁后精确路径补测记为 `PASS`；外部链接目标窗口与 Provider 授权内复制继续为 `NOT_TESTED`，不写成 PASS 或 FAIL。
