...
当前维护最新预览版。安全修复会优先进入最新预览版；在首个稳定版前不承诺旧预览版的长期补丁。
...
- 运行秘密按项目、环境和变量的不可逆索引分区保存；秘密正文只进入系统密钥库。SQLite 可以保存受限秘密引用、配置状态、非秘密值和元数据，但不保存秘密正文，面向前端的投影也不暴露秘密引用。
...
提交前运行 `pnpm check:secrets`。该检查只报告文件、行号和规则名，不打印疑似密钥内容；它是最低门禁，不能替代代码评审和托管平台的 Secret Scanning。
